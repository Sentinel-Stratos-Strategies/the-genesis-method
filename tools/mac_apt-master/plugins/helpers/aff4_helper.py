...
try:
    ...
    _HAVE_PYAFF4 = False
    container = lexicon = rdfvalue = None  # type: ignore
...
if _HAVE_PYAFF4:
    ...
    class EvidenceImageStream:
        def __init__(self, path_to_aff4_file):
            raise RuntimeError(
                "PyAFF4 is not available on this Python install. "
                "Add the vendor pyaff4 wheel from mac_apt other_dependencies or use Docker. "
                f"Tried path: {path_to_aff4_file!r}"
            )